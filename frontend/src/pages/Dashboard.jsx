/**
 * pages/Dashboard.jsx — SOC Command Center layout
 *
 * This file is intentionally thin — it only:
 *   - Uses the usePipeline hook for data/state
 *   - Uses alertUtils for derived values
 *   - Renders the layout and mounts child components
 *
 * No API calls, no business logic, no data transformations here.
 */

import { useState } from 'react'
import { RefreshCw, Shield, AlertTriangle, Settings } from 'lucide-react'

import { usePipeline }         from '../hooks/usePipeline'
import { countBySeverity, getTopSourceIPs, getCampaignCount } from '../services/alertUtils'
import { SEVERITY_LEVELS, ALERT_TABS } from '../constants/alerts'
...
const SEVERITY_COLORS = { Critical: 'purple', High: 'red', Medium: 'orange', Low: 'green' }

export default function Dashboard() {
  const { alerts, loading, running, error, lastRun, handleRunAnalysis } = usePipeline()
  const [isSettingsOpen, setIsSettingsOpen] = useState(false)
  const [activeTab, setActiveTab] = useState('alerts')

  const topIPs       = getTopSourceIPs(alerts)
  const campaignCount = getCampaignCount(alerts)
    ...
    <div style={{ background: '#0f172a', minHeight: '100vh', padding: 28, color: '#f8fafc', fontFamily: 'Segoe UI, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, borderBottom: '1px solid #334155', paddingBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            ...
            style={{ background: 'transparent', border: '1px solid #334155', color: '#94a3b8', borderRadius: 6, padding: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
          >
            <Settings size={18} />
          </button>
          <button
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        {SEVERITY_LEVELS.map(sev => (
          <StatCard key={sev} label={sev} value={countBySeverity(alerts, sev)} color={SEVERITY_COLORS[sev]} />
        ))}
      </div>
            ...
            : topIPs.map(([ip, cnt]) => (
              <div key={ip} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: Math.max(30, (cnt / (topIPs[0]?.[1] || 1)) * 120), height: 6, background: '#ef4444', borderRadius: 3 }} />
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
          {ALERT_TABS.map(tab => (
            ...
            >
              {tab}
              {tab === 'campaigns' && (
                <span style={{ background: '#ffffff22', borderRadius: 10, padding: '1px 7px', fontSize: 12 }}>
                  {campaignCount}
                </span>
              )}
            </button>
